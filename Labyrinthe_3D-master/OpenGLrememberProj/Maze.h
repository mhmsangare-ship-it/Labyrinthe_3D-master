...
// ── Cell size & wall height (fixed) ──────────────────────────────────
constexpr float CELL_SIZE = 2.0f;
constexpr float WALL_H = 2.5f;

// ── Trap: slows player to 30% speed ──────────────────────────────────
constexpr int TRAP_CELL = 2;   // grid value for a trap tile

class Maze
{
public:
    // Dynamic dimensions — set before calling generate()
    int cols = 13;
    int rows = 13;

    // Flat grid: 0=passage, 1=wall, 2=trap
    std::vector<int> grid;
